...
最快路径——一条命令从零到一个已填充的 console：

```bash
node src/cli.js quickstart --out /path/to/target-repo --name my-workflow
```

`quickstart` 会生成 `opt.workflow.json`、编译全部 harness 资产,并跑一遍把第一个阶段预先批准的 demo orchestration,然后打印 `.tpan-opt-co-worker/console/index.html` 的路径。打开它,console 已经显示实时阶段进度——第一个阶段 done、下一个阶段作为打开的工单——所以你在学习任何单条命令之前,就先看到系统在运转。加 `--no-demo` 只生成与编译、不跑 demo;或用 `--template production-feature` 选更完整的交付 workflow。

如需更细的控制,底层命令依然可用。在目标仓库中创建 starter workflow：
