...
本文档中更完整的 team operating system 表述代表产品方向。执行编排器负责路由和门控，并可通过 opt-in、harness-neutral 的 `--invoke`/`--agent-command` 适配器驱动当前阶段的 owner agent，但还不会自主调度或协同多个 agent。托管式编排、marketplace package 安装闭环和 YAML authoring 目前还不属于当前 package 的实现范围。
