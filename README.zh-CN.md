...
它会 clone 仓库并自动注册 `co-worker` MCP server(通过 `.mcp.json`)—— 无需 npm install、无需手动配置。然后只管对你的 agent 说话;完整闭环就是五步：

1. **Scaffold** —— “用 `co_worker_quickstart` 在 `./my-repo` 里建一条 workflow。”它会生成 `opt.workflow.json`、编译全部 harness 资产、seed 一次 demo run,并打印 console 路径。
2. **看到它** —— 打开打印出的 `./my-repo/.tpan-opt-co-worker/console/index.html`。console 已填充——第一个阶段 `done`、下一个阶段是打开的工单。
3. **Next** —— “对 `./my-repo` 调用 `co_worker_next`。”它返回打开的工单和下一步动作。
4. **Approve** —— “对 `./my-repo` 调用 `co_worker_approve`,批准 `plan` 阶段的 `scope_confirmed` gate,审批人是我。”它会把批准记为证据、推进编排器,并打印下一张工单。
5. **重复** —— 不断让它 `co_worker_next` → `co_worker_approve`,直到每个阶段都 `done`。

**Codex / Cursor / 其他 MCP agent** —— 先 clone 仓库,再把 agent 指向本地 server。具体的 `config.toml` / JSON 见[以插件方式安装（MCP）](#以插件方式安装mcp),然后让 agent 跑同一条 `co_worker_quickstart` → `co_worker_next` → `co_worker_approve` 链路。

### 选项 B —— 直接用 CLI

快速开始就是一条命令——从空目录到一个能用、已填充的 console：

```bash
node src/cli.js quickstart --out /path/to/target-repo --name my-workflow
```

它会生成 `opt.workflow.json`、编译全部 harness 资产,并跑一遍把第一个阶段预先批准的 demo orchestration。打开它打印出的路径：

```bash
open /path/to/target-repo/.tpan-opt-co-worker/console/index.html
```

console 打开时已填充——第一个阶段 `done`、下一个阶段作为打开的工单——所以你在学习任何其他命令之前,就先看到系统在运转。加 `--no-demo` 跳过 demo run;或用 `--template production-feature` 选更完整的交付 workflow。

接着用 CLI 驱动 workflow。下面三条命令就是日常的全部闭环——看进度、取下一张工单、批准 gate 推进——全程不用手改证据文件(它们与上面的 `co_worker_*` MCP 工具共享同一套核心,所以命令行与 agent 内行为完全一致)：

```bash
node src/cli.js status   --out /path/to/target-repo   # 每个阶段当前进展
node src/cli.js next     --out /path/to/target-repo   # 打开的工单 + 下一步动作
node src/cli.js approve scope_confirmed --stage plan --by you@example.com --out /path/to/target-repo
```

`approve` 会把审批人记为证据、推进编排器,并打印下一张工单——所以 `next` → `approve` → `next` 就是你重复执行、直到每个阶段都 `done` 的闭环。
...
- **Skill / MCP / Hook 注册中心** —— `node src/cli.js catalog` 列出 templates、policy packs 和 teams;`node src/cli.js marketplace` 列出 skills、MCP servers 和 hooks;`node src/cli.js presets` 列出 gate presets。
...
- **`mode: opt`**(默认)—— 交给 code agent。用编排器驱动它们:
  `node scripts/orchestrate-workflow.mjs --invoke --loop` 会自动逐阶段推进,直到遇到人工门禁,并在每一步把该步骤的 skills/MCP/hooks 作用域下放给 agent。
- **`mode: team`** —— 交给人类队友。编译会生成 **`PLAYBOOK.md`**,一份可直接照着做的清单,每个人在自己的产品或模块上跑完整流程。用 **`tpan-opt-co-worker dashboard`** 把所有人并排汇总(每个模块一次带标签的运行)。
...
`wizard` 是交互式编排路径:它依次询问模板、可选复用团队、policy packs、MCP server、按 role 的 MCP 分配,以及生命周期 hook,然后写出 `opt.workflow.json` 并把全部 harness 资产(包括 `.mcp.json`、`.codex/config.toml` 的 MCP 接线和 `.claude/settings.json` 的 hook)编译到 `--out`。每个 `[默认值]` 直接回车即可接受;选择题可按编号或 id 作答。它产出的 schema 见[工作流节点：MCP server 与 hook](#工作流节点mcp-server-与-hook)。它默认拒绝覆盖已有文件,需要时显式传入 `--force`。
