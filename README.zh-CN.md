...
TPAN-OPT/CO-WORKER 是一个无外部依赖的 Node.js CLI。需要 Node.js 22+ 和 npm 10+。
...
- **OPT Runtime** —— `node scripts/orchestrate-workflow.mjs --run-id local` 把各阶段在不同 owner 间路由,并产出下一张工单(这正是 quickstart demo 所 seed 的)。
