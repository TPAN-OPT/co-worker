...
**用真实 agent 跑同一条流程。** quickstart 的阶段 gate 是 agent-neutral 的——只要该阶段的工件出现在 `.tpan-opt-co-worker/artifacts/<stage>.md`,无论谁写的,gate 就通过。所以把编排器指向真实 agent CLI 并让它写到那里,同样的四个阶段会逐级推进,并仍然停在唯一的人工 gate:
...
console 显示每个阶段都 `done`、一张填满的 agent 调用记录表,以及唯一一张打开的工单——你的审批——所以你在学习任何其他命令之前,就先看到一支 agent 团队真的交付了工作。它们写出的工件在 `.tpan-opt-co-worker/demo/artifacts/`。加 `--no-demo` 只 scaffold 不跑团队;或用 `--template production-feature` 选带真实检查 gate 的交付 workflow。该命令还会打印一行可直接粘贴的命令,用真实 agent CLI 替代内置 demo agent 重跑同一条流程。
