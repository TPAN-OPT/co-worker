...
TPAN-OPT/CO-WORKER 是 **AI agent 产出的治理层（governance layer）**。它把团队的交付方式变成可版本化、可执行的工作流：在多个 AI 编码 agent 之间路由工作，为每个阶段设置真实的检查门禁，并保留一条可审计的证据链。AI 让所有人更快——co-worker 让这份"快"变得**可审计、可复现、可信任**。

它最锋利的用法是 **OPT —— One Person Team**：一个负责人带一支专业 AI agent 团队（planner、engineer、reviewer、release lead），跑一套受治理的流程——交付过去需要一整个团队才能完成的工作，同时仍然通过一个真实团队应有的门禁、审批和证据。

第一次来？先看 **[定位与路线图](docs/POSITIONING.md)**（面向谁、往哪走），或跑一遍 **[Hero demo](docs/HERO-DEMO.md)**，亲眼看到一次糟糕的改动如何在 gate 处被拦下。

### 它和别的有什么不同

任何框架都能*运行* agent。co-worker 治理的是它们的**产出**——而且是 harness 中立的,叠加在你已经在用的任何 agent 之上。

| 你已经有的 | co-worker 补上的 |
| --- | --- |
| 一个把 agent 接起来、跑起来的框架（CrewAI、LangGraph、AutoGen、Claude subagents） | 叠在其上的治理层:版本化 workflow、真实的 check gate、人工审批,以及可审计的证据轨迹——适用于任意 agent |

别只听我们说:去翻一份 **[真实、已提交的证据轨迹](docs/sample-evidence/)**（agent 运行 → gate 变绿 → 停在人工 gate → 审批被记录),无需安装任何东西。
...
1. **Scaffold 并跑起团队** —— “用 `co_worker_quickstart` 在 `./my-repo` 里建一条 workflow。”它会生成 `opt.workflow.json`、编译全部 harness 资产,并把一支四角色团队(planner → engineer → reviewer → lead)端到端跑一遍。**默认情况下这是内置的_离线 demo agent_：一次对整条闭环的 dry-run 预览,产出的工件是带标注的占位内容,并非真实工作**——它不需要 agent CLI、不联网、不产生 API 费用,让你能瞬间看到整条受治理的流程。加 `--real`(或工具参数 `real`)即可驱动已安装的 agent CLI 产出真实结果。无论哪种方式,每个阶段都会写出一份工件,整条 run 停在唯一的人工审批 gate。
